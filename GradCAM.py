...
class GradCAM:
    ...
    def scale_cam_image(self, cam, target_size=None):
        ...
        for img in cam:
            ...
            if target_size is not None:
                # 插值
                target_x = np.linspace(0, img.shape[0] - 1, target_size[0])
                img = np.interp(target_x, np.arange(img.shape[0]), img)
            result.append(img)
    ...
    def compute_cam_per_layer(
        ...
        for i in range(len(self.target_layers)):
            ...
            if size is None:
                scaled = self.scale_cam_image(cam, (target_size[0], 1))
            else:
                scaled = self.scale_cam_image(cam, (size, 1))
    ...
    def __call__(self,
        ...
        return self.forward(input_tensor,
                            targets, eigen_smooth, size=size)
